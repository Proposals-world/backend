...
function categorizeDetails(profile) {
    return {
        "{{ __('userDashboard.likeMe.personal') }}": {
            ...
            "{{ __('userDashboard.likeMe.children') }}": profile.profile.children ?? "{{ __('userDashboard.likeMe.na') }}",
        ...
        "{{ __('userDashboard.likeMe.professional') }}": {
            ...
            "{{ __('userDashboard.likeMe.employment_status') }}": profile.profile.employment_status ? "{{ __('userDashboard.likeMe.employed') }}" : "{{ __('userDashboard.likeMe.unemployed') }}",
        ...
        "{{ __('userDashboard.likeMe.lifestyle') }}": {
            "{{ __('userDashboard.likeMe.hijab_status') }}": profile.profile.hijab_status !== null
                ? (profile.profile.hijab_status ? "{{ __('userDashboard.likeMe.yes') }}" : "{{ __('userDashboard.likeMe.no') }}")
                : "{{ __('userDashboard.likeMe.na') }}",
            "{{ __('userDashboard.likeMe.smoking_status') }}": profile.profile.smoking_status !== null
                ? (profile.profile.smoking_status ? "{{ __('userDashboard.likeMe.yes') }}" : "{{ __('userDashboard.likeMe.no') }}")
                : "{{ __('userDashboard.likeMe.na') }}",
    ...
    Object.entries(details).forEach(([category, fields]) => {
        ...
        Object.entries(fields).forEach(([label, value]) => {
            ...
            if (Array.isArray(value)) {
                ...
                container.append(`<div class="col-md-6 mb-3"><strong>${label}:</strong><br>${badges}</div>`);
            } else {
                container.append(`<div class="col-md-6 mb-2"><strong>${label}:</strong> ${value}</div>`);
            }
        });
    });
}
...
$(document).on('click', '.profile-card', function () {
    ...
    if (!user.contact_exchanged) {
        $('#revealContactBtn').removeClass('d-none').off('click').on('click', function () {
